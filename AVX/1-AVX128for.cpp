...
int fillImage(Mandelbrot *mbrot)
{
    ...
    for (int yi = 0; yi < heigth; ++yi, PIXELS(mbrot) += width)
    {
        ...
        for (int xi = 0; xi < width; xi += 4, x00 = _mm_add_ps(x00, _mm_mul_ps(dx, _4)))
        {
            ...
            int pix = pn[0]; 
            Pixels[xi + 0]  = 0xFF000000 + sin(pix) * (2 << 20) + pow(pix, 2) * (2 << 11) + tan(pix) * (2 << 15);
            
            pix = pn[1]; 
            Pixels[xi + 1]  = 0xFF000000 + sin(pix) * (2 << 20) + pow(pix, 2) * (2 << 11) + tan(pix) * (2 << 15);
            
            pix = pn[2]; 
            Pixels[xi + 2]  = 0xFF000000 + sin(pix) * (2 << 20) + pow(pix, 2) * (2 << 11) + tan(pix) * (2 << 15);
            
            pix = pn[3]; 
            Pixels[xi + 3]  = 0xFF000000 + sin(pix) * (2 << 20) + pow(pix, 2) * (2 << 11) + tan(pix) * (2 << 15);
        }
    }
